...
- **Implementation note (2026-08-24)**: The local hybrid mechanism is present and currently selected by default, but the repository has not recorded the lexical-versus-hybrid benchmark results required below. This implementation state does not satisfy the activation gate and remains provisional. The current first `build-index` can also download FastEmbed model weights when absent; an explicit setup command is still required to meet the offline-construction contract.
