...
```mermaid
graph TD
    A["Evaluate Current FTP Status"] --> B{"Is athlete early season or post-detraining?"}
    
    B -- Yes --> C["Start Threshold / Sweet Spot Progression"]
    C --> C1{"Pacing / Feel Confident?"}
    C1 -- Novice/Uncertain --> C2["Progress Time-in-Zone First (e.g. 4x10 -> 3x15 -> 2x20)"]
    C1 -- Experienced --> C3["Auto-regulate Power & Extend Final Interval if Strong"]
    
    B -- No --> D{"Is FTP increasing with progressive threshold work?"}
    
    D -- Yes --> E["Continue Threshold Progression until TTE reaches 50-70+ mins"]
    D -- No --> F{"Assess Failure Mode & Recovery"}
    
    F -- High Fatigue / Life Stress --> G["Reduce Intensity; Maintain with Submaximal Sweet Spot"]
    F -- TTE Maxed (60-90m) & FTP Flat --> H{"Is VO2max Ceiling Limiting FTP?"}
    
    H -- Ceiling Limited --> I["Switch to High-Intensity VO2max Block"]
    H -- Ceiling Maxed / Plateaued --> J["Pivot to Repeatability, Durability, Sprint & Racecraft"]
    
    F -- Dense In-Season Racing --> K["Minimal Dose Maintenance: 1 Threshold/Sweet Spot Session every 2-4 Wks"]
```
