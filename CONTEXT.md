...
- **Passage Retrieval**: Finding and ranking Evidence Passages. The current implementation explores 20 SQLite FTS5 BM25 and local BGE-small `sqlite-vec` candidates, fuses them, and returns only passages that meet a server-side relevance policy (at most 20). Its activation as the approved default remains provisional because the required corpus-specific lexical-versus-hybrid benchmark results have not yet been recorded.
...
- **Grounded Synthesis**: An explanation, comparison, or plan produced from retrieved Evidence Passages by either the application-owned AI workflow or a connected MCP client, with source claims traceable to citations and inference kept distinguishable from source evidence. Passage Retrieval remains local; an application-configured synthesis provider receives only the Athlete Query and selected Evidence Passages needed to produce the answer.
- **Direct Search**: The model-free application workflow that turns an Athlete Query into ranked Knowledge Sources and Evidence Passages for immediate inspection. It remains available without an external AI provider.
- **AI Search**: The optional application-owned workflow that performs Passage Retrieval and asks a configured hosted LLM to produce Grounded Synthesis as Markdown for controlled rendering. The backend owns retrieval, evidence selection, and answer validation; when no synthesis provider is configured, AI Search is unavailable rather than silently degraded.
- **Knowledge Source**: A curated English article, podcast note, or book included in the Knowledge Base.
- **Knowledge Source Revision**: An immutable editorial snapshot of one Knowledge Source, including its Markdown content and validated metadata. Editing creates a new revision rather than mutating a published revision.
- **Published Revision**: The single Knowledge Source Revision currently visible in the Library and eligible for Passage Retrieval. Publishing advances this pointer atomically; archiving removes the Knowledge Source from discovery without destroying its revision history.
...
The repository currently defaults to the implemented hybrid path, but this precedes the benchmark evidence required by ADR 0002. The 32-query labelled dataset has integrity coverage; its lexical baseline, hybrid quality metrics, latency, negative-query results, and selection-policy calibration remain unmeasured. Until those results demonstrate the required improvement, FTS5 is the approved architectural baseline and hybrid default activation is provisional. The retrieval path runs locally after setup; the current first `build-index` may download FastEmbed model weights when they are absent, which remains an explicit-setup contract gap.
